...
class ClipProvider:
    """
    Applies the rule pipeline and returns CLIP conditioning.
    """
    ...
    @classmethod
    def IS_CHANGED(cls, clip: Any, positive: str, negative: str):
        return Pipeline.DEFAULT.get_cache_key(positive, negative)

    def process(self, clip: Any, positive: str, negative: str):
        ...
        positive_tokens = clip.encode_from_tokens_scheduled(clip.tokenize(positive))
        negative_tokens = clip.encode_from_tokens_scheduled(clip.tokenize(negative))
        return (positive_tokens, negative_tokens)


class TextProvider:
    """
    Applies the rule pipeline and returns processed prompt text.
    """
